...
[ -f "$MACOS_DIR/tools/allure-commandline/bin/allure" ] || CHK_ERR="便携 allure-commandline"
if [ "${SKIP_JRE:-}" != "1" ]; then
...
TOOLS_MB=$(du -sm "$MACOS_DIR/tools" 2>/dev/null | cut -f1 || echo "?")
...
============================================
 ...
 拷贝到别的 mac 上运行前:
   unzip -q $OUT_ZIP && xattr -cr PomeloTool.app && open PomeloTool.app
